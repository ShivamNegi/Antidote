...
function generateMap(apiPath, containerId) {
    ...
    function renderHeatmap(markers) {
        ...
        markers.forEach(function (marker) {
            ...
            // coord.push(new  google.maps.visualization.WeightedLocation(new google.maps.LatLng(marker.loc), Math.floor((Math.random() * 10) + 1)));
        });
    ...
    function renderEvent(addr, title) {
        geocoder.geocode({
            ...
            if (status == 'OK') {
                ...
                google.maps.event.addListener(m, 'click', function () {
                    var info = new google.maps.InfoWindow();
                    info.setContent('<b>Address:</b> ' + addr + '<br>' + '<b>PIN:</b> ' + title + '<br>');
                    info.open(map, this);
                });
            } else {
    ...
    function dataSentinel(data) {
        ...
        var north_lat = 70.09234869140867; // top
        var west_lng =  19.04326530273601; // left
        var east_lng =  31.60494895970077; // right
        var south_lat = 59.76349858275872; // bottom
